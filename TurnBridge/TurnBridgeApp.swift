...
struct VBridge: App {
    ...
    func increaseTunnelThreads(by delta: Int, completionHandler: @escaping (Bool) -> Void) {
        let sanitizedDelta = max(delta, 1)
        NETunnelProviderManager.loadAllFromPreferences { tunnelManagersInSettings, error in
            if let error = error {
                NSLog("Error (loadAllFromPreferences): \(error)")
                SharedLogger.error("Failed to load tunnel preferences: \(error.localizedDescription)")
            ...
            guard let tunnelManager = tunnelManagersInSettings?.first else {
                SharedLogger.warning("No tunnel manager found for thread increase")
                completionHandler(false)
                return
            }
            ...
            guard session.status == .connected else {
                SharedLogger.warning("Thread increase requested while tunnel is not connected")
                completionHandler(false)
                return
            }

            let payload: [String: Any] = [
                "command": "increaseThreads",
                "delta": sanitizedDelta
            ]

            guard let data = try? JSONSerialization.data(withJSONObject: payload, options: []) else {
                SharedLogger.error("Failed to encode thread increase payload")
                completionHandler(false)
                return
            }

            do {
                SharedLogger.info("Requesting \(sanitizedDelta) extra tunnel thread(s)")
                try session.sendProviderMessage(data) { responseData in
                    guard
                        ...
                        SharedLogger.error("Thread increase request rejected by provider")
                        completionHandler(false)
                        return
                    }
                ...
                SharedLogger.error("Failed to send thread increase request: \(error.localizedDescription)")
                completionHandler(false)
            }
        }
    }
