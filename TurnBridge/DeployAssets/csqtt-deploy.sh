...
cleanup_firewall_rules() {
    ...
    iptables -t nat -C POSTROUTING -s "$CLIENT_CIDR" -o "$WAN_IFACE" -j MASQUERADE 2>/dev/null && \
        iptables -t nat -D POSTROUTING -s "$CLIENT_CIDR" -o "$WAN_IFACE" -j MASQUERADE || true
    ...
    iptables -t mangle -C FORWARD -s "$CLIENT_CIDR" -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu 2>/dev/null && \
        iptables -t mangle -D FORWARD -s "$CLIENT_CIDR" -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu || true

    iptables -t mangle -C FORWARD -d "$CLIENT_CIDR" -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu 2>/dev/null && \
        iptables -t mangle -D FORWARD -d "$CLIENT_CIDR" -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu || true
}
...
if [ -n "${CSQTT_PASSWORD:-}" ]; then
    ARGS="$ARGS --password ${CSQTT_PASSWORD}"
fi
...
CSQTT_PASSWORD=$TUNNEL_PASS
CSQTT_WEB_USER=$WEB_USER
CSQTT_WEB_PASS=$WEB_PASS
EOF

    cat >"$CONFIG_DIR/csqtt.env" <<EOF
CSQTT_PEER_PORT=$PEER_PORT
CSQTT_WEB_PORT=$WEB_PORT
CSQTT_CLIENT_CIDR=$CLIENT_CIDR
CSQTT_ENABLE_TCPMSS=$ENABLE_TCPMSS
