...
struct LogView: View {
        ...
        entries.filter { entry in
            ...
            if !searchText.isEmpty {
                ...
                return entry.message.lowercased().contains(query)
                    ...
                    || entry.level.label.lowercased().contains(query)
            }
    ...
    var body: some View {
        ...
        .onAppear {
            entries = SharedLogger.readEntries()
            ...
            if showFilters {
                VStack(spacing: 8) {
                    ...
                    HStack(spacing: 6) {
                        ...
                        filterChip("All", isActive: selectedSource == nil) {
                            selectedSource = nil
                        }
                        ForEach(LogSource.allCases, id: \.self) { source in
                            filterChip(source.displayName, isActive: selectedSource == source) {
                    ...
                    HStack(spacing: 6) {
                        ...
                        ForEach(LogLevel.allCases, id: \.self) { level in
                            filterChip("\(level.icon) \(level.label)", isActive: minimumLevel == level) {
                                minimumLevel = level
                            }
                        }
                        Spacer()
